...
A container image packages user-space software, libraries, and selected configuration. It does not include a separate hardware machine and normally shares the host Linux kernel. Apptainer is designed for multi-user high-performance computing and normally runs rootless. That means the process keeps your RCC user and group access instead of gaining unrestricted root on the host, and it does not depend on a privileged Docker-style daemon. This is important on shared compute nodes: the host kernel, drivers, devices, file permissions, and Slurm allocation remain under RCC control, while a mistaken or compromised application is limited to the access already held by the submitting user. Rootless does not mean harmless or isolated from your data. The container can still read, change, or delete visible files that your user can access, so bind only the paths the job needs and make source data read-only where possible. The image is not the correct place for active project data, credentials, or patient information. Containers improve reproducibility of software, but they do not validate algorithms, reference data, parameters, or scientific interpretation. Building an image is a separate operation and must use the RCC-approved rootless, fakeroot, or remote-builder pattern.
