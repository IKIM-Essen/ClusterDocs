...
> **Five rules for the planned supported service:**
>
> 1. Run **one Nextflow controller per analysis** on an RCC interactive node
>    (`shellhost`), normally in `tmux`.
...
- explain the difference between the Nextflow controller and a process;
- start one persistent Nextflow controller in `tmux` after release;
- use the RCC launcher without replacing institutional site configuration;
...
Your laptop
  -> login.ikim.uk-essen.de
       -> RCC interactive node (shellhost)
            -> one Nextflow controller
                 -> Slurm
                      -> task A on an RCC compute node
                      -> task B on an RCC compute node
                      -> task C on an RCC compute node
...
> Once released: one controller on `shellhost`; Slurm for every compute task;
